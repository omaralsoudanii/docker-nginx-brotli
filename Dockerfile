...
# Google brotlie repo latest commit
...
# Clone brotli to the commit hash we defined, grab NGINX source code
RUN \
...
# install NGINX, and it's modules in addition to brotli
RUN \
    cd /usr/src/nginx-$NGINX_VERSION \
    && make install \
